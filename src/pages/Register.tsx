...
const RegisterPage: React.FC = () => {
  ...
  const onSubmit = async (values: Form) => {
    try {
      const res = await apiRegister(values.email, values.password);
      setAuth(res.access_token, res.organisation_id);
      const me = await getMe();
      setUser(me);
      navigate('/');
    } catch {
      show({ title: 'Registration failed', description: 'That email may already be in use.', variant: 'error' });
    }
  };
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-primary/10 via-warm-off to-emerald-50 px-4">
      <div className="w-full max-w-md">
        <div className="mb-8 text-center">
          ...
          <p className="mt-1 text-sm text-slate-500">Your organisation is created automatically</p>
        </div>

        <div className="rounded-2xl bg-white p-8 shadow-xl ring-1 ring-slate-200">
          <h2 className="mb-6 text-lg font-semibold text-slate-900">Create your account</h2>
